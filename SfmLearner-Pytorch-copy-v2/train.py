...
from loss_functions import photometric_reconstruction_loss, explainability_loss, smooth_loss, add_photometric_reconstruction_loss
...
#    pose_exp_net = torch.nn.DataParallel(pose_exp_net)
    ...
    with open(args.save_path/args.log_full, 'w') as csvfile:
        writer = csv.writer(csvfile, delimiter='\t')
        writer.writerow(['train_loss', 'photo_loss', 'explainability_loss', 'smooth_loss', 'add_photo_loss',])
...
def train(args, train_loader, disp_net, pose_exp_net, optimizer, epoch_size, logger, tb_writer):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(train_loader):
        ...
        disparities_ref = disp_net(ref_imgs[0])
        depth_ref = [1/disp for disp in disparities_ref]
        loss_4 = add_photometric_reconstruction_loss(ref_imgs,
                                                               intrinsics, depth_ref,
        ...
        if log_losses:
            ...
            tb_writer.add_scalar('add_photometric_error', loss_4.item(), n_iter)
...
def validate_without_gt(args, val_loader, disp_net, pose_exp_net, epoch, logger, tb_writer, sample_nb_to_log=3):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(val_loader):
        ...
        loss_1, warped, diff = photometric_reconstruction_loss(tgt_img, ref_imgs,
                                                               ...
                                                               args.rotation_mode, args.padding_mode)
        loss_1 = loss_1.item()
        depth_ref = 1/disp_net(ref_imgs[0])
        loss_4 = add_photometric_reconstruction_loss(ref_imgs,
                                                               intrinsics, depth_ref,
                                                               explainability_mask, pose,
                                                               args.rotation_mode, args.padding_mode)
    ...
    return losses.avg, ['Validation Total loss', 'Validation Photo loss', 'Validation Exp loss', 'Validation Additional Photo loss']
